...
public class Memo extends Timestamped { // 생성,수정 시간을 자동으로 만들어줍니다.
    ...
    @OneToMany(fetch = FetchType.LAZY, cascade = CascadeType.ALL, orphanRemoval = true)  //부모가 삭제될 때 자식들도 다 삭제되는 어노테이션
    @JsonIgnore
    @JsonManagedReference //DB연관관계 무한회귀 방지
    private List<Comment> commentList;


    @OneToMany(mappedBy = "memo", cascade = CascadeType.ALL, orphanRemoval = true)  //부모가 삭제될 때 자식들도 다 삭제되는 어노테이션
    @JsonIgnore
    @JsonManagedReference //DB연관관계 무한회귀 방지
